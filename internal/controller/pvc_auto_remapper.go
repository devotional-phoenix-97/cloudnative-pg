...
func (r *ClusterReconciler) waitForDeletion(
	...
	return retry.Do(
		...
		retry.Attempts(deleteTimeoutSeconds),
...
func (r *ClusterReconciler) deletePVC(ctx context.Context, namespacedName types.NamespacedName) error {
	...
	if err := r.Patch(ctx, patchedPvc, client.MergeFrom(&pvc)); err != nil {
		contextLogger.Error(err, "failed to remove labels, annotations and owner references from PVC")
		return err
	}
	const (
		patchTimeoutSeconds  = 60
	...
	err := retry.Do(
		func() error {
			...
			if len(pvc.OwnerReferences)+len(pvc.Labels) > 0 {
				return fmt.Errorf("still owner references, and/or labels")
			}
			return nil
		},
		retry.Attempts(patchTimeoutSeconds),
		retry.Delay(time.Second),
		retry.DelayType(retry.FixedDelay),
	)
	if err != nil {
...
func (r *ClusterReconciler) cleanRemappedPVCs(
	ctx context.Context,
	toRemap pvcremapper.InstancePVCs,
) error {
	contextLogger := log.FromContext(ctx)
	for _, ipvc := range toRemap {
		contextLogger.Info("cleaning old pvc", "pvc", ipvc.AsNamespacedName())
		if err := r.deletePVC(ctx, ipvc.AsNamespacedName()); err != nil {
			return err
		}
	}
	return nil
...
func (r *ClusterReconciler) deletePod(
	...
	if err != nil {
		contextLogger.Error(err, "failed to remove labels, annotations and owner references from pod", "pod", podName)
		return err
	}
	err = retry.Do(
		func() error {
			...
			if len(pod.OwnerReferences)+len(pod.Labels)+len(pod.Annotations) > 0 {
				return fmt.Errorf("still owner references, labels, and/or annotations")
			}
			return nil
		},
		retry.Attempts(patchTimeoutSeconds),
...
func (r *ClusterReconciler) clonePVC(
	...
	err = retry.Do(
		...
		retry.Attempts(createTimeoutSeconds),
		retry.Delay(time.Second),
		retry.DelayType(retry.FixedDelay),
	)
	if err != nil {
...
func (r *ClusterReconciler) pvcRemapping(
	...
	source := ipvc.AsNamespacedName()
	if r.deletePVC(ctx, source); err != nil {
...
func (r *ClusterReconciler) instanceRemapping(
	...
	return r.cleanRemappedPVCs(ctx, toRemap)
}
